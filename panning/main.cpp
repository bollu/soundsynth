...
public:
  ...
  void GenerateSample(float *out, const float pan) {
...
int main() {
  ...
  float pan = -1; 
  float pandir = 1.3e-5;
  for (int i = 0; i < c_numSamples; i+= 2) {
    pan += pandir;
    if (pan < -1) { pandir = -pandir; pan = -1; }
    if (pan > 1) { pandir = -pandir; pan = 1; }

    samples[i] = samples[i+1] = 0;
    if (i < c_numSamples/42) {
        notes[0].GenerateSample(samples.data() + i, pan);
    } else if (i < 20 * c_numSamples/42){
        notes[0].GenerateSample(samples.data() + i, pan);
        notes[1].GenerateSample(samples.data() + i, pan);
    } else if (i < 21 * c_numSamples/42) {
        notes[0].GenerateSample(samples.data() + i, pan);
        notes[1].GenerateSample(samples.data() + i, pan);
        notes[2].GenerateSample(samples.data() + i, pan);
    } else {
        notes[0].GenerateSample(samples.data() + i, pan);
        notes[1].GenerateSample(samples.data() + i, pan);
        notes[3].GenerateSample(samples.data() + i, pan);
    }
