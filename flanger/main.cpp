...
class Drum {
public:
  Drum(float frequency, float sampleRate, float feedback) {
...
template<typename T>
T lerpIx(T *xs, float ix) {
    int l = floor(ix);
    float dl = ix - l;
    return (1 - dl) * xs[l] + dl*xs[l+1];
}
...
int main() {
  ...
  std::vector<Drum> notes;
  for(int i = 0; i < 8; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 0),
                  float(c_sampleRate), 0.950f));
  }
  for(int i = 0; i < 8; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 2),
                  float(c_sampleRate), 0.98f));
  }
  for(int i = 0; i < 8; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 3),
                  float(c_sampleRate), 0.98f));
  }
  for(int i = 0; i < 4; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 0),
  ...
  for(int i = 0; i < 2; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 1.8),
                  float(c_sampleRate), 0.999f));
  }
  for(int i = 0; i < 2; ++i) {
      notes.push_back(Drum(CalcFrequency(3, 0),
                  float(c_sampleRate), 0.999f));
  }
  const int NNOTES = notes.size();

  int c_numSamples = 1.8e5;
  for (int i = 0; i < c_numSamples; ++i) {
    float out = 0;
    for(int j = 0; j <  (i*NNOTES)/c_numSamples; ++j) {
        out += notes[j].GenerateSample();
    }
    // for (Drum &n : notes) { samples[i] += n.GenerateSample(); }
    // to keep from clipping
    out *= 0.5f;
    samples.push_back(out);
  }

  for (int i = 0; i < 2*c_numSamples; ++i) {
    float out = 0;
  ...
  const float flangeAngularVel = 1e-3;
  const int flangeDepth = 100;
  std::vector<float> samplesFlanged;
  for(int i = 0; i < flangeDepth; ++i) {
      samplesFlanged.push_back(samples[i]);
  }
  float flangeTheta = 0;
  for(int i = flangeDepth+1; i < samples.size(); ++i) {
      flangeTheta += flangeAngularVel;
      samplesFlanged.push_back(samples[i] + lerpIx(samples.data(), i + flangeDepth * sin(flangeTheta)));
  }
  ...
  playBufferInt16(samplesFlanged.data(), samplesFlanged.size(), c_sampleRate, c_numChannels);
