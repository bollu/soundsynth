...
int main() {
  ...
  for(int i = 0; i < NNOTES/4; ++i) {
      notes.push_back(Drum(CalcFrequency(0, 2*(rand() % 5)),
                  float(c_sampleRate), 0.999f));
  }
  for(int i = 0; i < NNOTES/4; ++i) {
      notes.push_back(Drum(CalcFrequency(1, 2*(rand() % 5)),
                  float(c_sampleRate), 0.999f));
  }
  for(int i = 0; i < NNOTES/4; ++i) {
      notes.push_back(Drum(CalcFrequency(2, 2*(rand() % 5)),
                  float(c_sampleRate), 0.999f));
  }
  for(int i = 0; i < NNOTES/3; ++i) {
      notes.push_back(Drum(CalcFrequency(2, 2*(rand() % 5)),
                  float(c_sampleRate), 0.999f));
  }
  ...
  for (int i = 0; i < c_numSamples; ++i) {
    ...
    for(int j = 0; j < (i*NNOTES/c_numSamples); ++j) {
        samples[i] += notes[j].GenerateSample();
    }
