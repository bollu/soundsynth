...
void GenerateSamples(std::vector<float> &samples, int sampleRate) {
  ...
  enum ESongMode { e_twinkleTwinkle, e_strum };
  ...
  for (int i = 0; i < samples.size(); ++i) {
    ...
    if (mode == e_twinkleTwinkle) {
      // why div 2?
      const int c_noteTime = sampleRate / 2;
      int time = i - timeBegin;
      // if we should start a new note
      if (time % c_noteTime == 0) { // start a new node.
        ...
        // case 3: {
          ...
          notes.push_back(CKarplusStrongStringPluck(CalcFrequency(3, 7),
                                                    float(sampleRate), 0.996f));
          break;
        }
        // }
        ...
        // case 5: {
          ...
          notes.push_back(CKarplusStrongStringPluck(CalcFrequency(3, 9),
                                                    float(sampleRate), 0.996f));
        }
        //   break;
        // }
    ...
    // case e_strum: {
    else if (false && mode == e_strum) {
      const int c_noteTime = sampleRate / 32;
      int time = i - timeBegin - sampleRate;
      // if we should start a new note
      if (time % c_noteTime == 0) {
        ...
        case 0:
          notes.push_back(
              CKarplusStrongStringPluck(55.0f, float(sampleRate), 0.996f));
          break;
        case 1:
          notes.push_back(CKarplusStrongStringPluck(55.0f + 110.0f,
                                                    float(sampleRate), 0.996f));
          break;
        case 2:
          notes.push_back(CKarplusStrongStringPluck(55.0f + 220.0f,
                                                    float(sampleRate), 0.996f));
          break;
        case 3:
          notes.push_back(CKarplusStrongStringPluck(55.0f + 330.0f,
                                                    float(sampleRate), 0.996f));
          break;
        case 4:
          mode = e_strum;
